...
file_list = os.listdir('/data3/whb/data/Moment-10m/videos')
for file in file_list:
    file_path = os.path.join('/data3/whb/data/Moment-10m/videos/', file)
    video = read_video_file(file_path, decoder='decord', decode_audio=False)
    frames, duration = read_video_frames(
        video, num_samples=128, centered=False
    )
    frames = frames.transpose(0, 1)
    print(file_path, frames.shape, duration)