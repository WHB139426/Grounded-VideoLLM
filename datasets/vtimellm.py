...
class VTimeLLM_Stage3(Dataset):
    def __init__(
        ...
        self.didemo_video_path = didemo_video_path
        self.num_frames = num_frames
        self.num_segs = num_segs
        self.num_temporal_tokens = num_temporal_tokens
        self.sample = sample
        ...
        for item in self.data:
            ...
            self.text_inputs.append(self.chat_template.encode(conversations))
            self.dataset_names.append('vtimellm-stage3-'+item['source'])
    ...
    def __getitem__(self, index):
        ...
        if dataset_name == 'vtimellm-stage3-anet':
            video_path = os.path.join(self.anet_video_path, video_file)
        elif dataset_name == 'vtimellm-stage3-didemo':
            video_path = os.path.join(self.didemo_video_path, video_file)
...
# dataset = VTimeLLM_Stage3(llm='llama3')
# for i in range(100):
#     entry = random.choice(dataset)
