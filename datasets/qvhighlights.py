...
class QVHighlights(Dataset):
    def __init__(
        ...
        for item in self.data:
            self.question_ids.append(item['qid'])
            self.video_files.append(item['vid']+'.mp4')
            self.video_ids.append(item['vid'])
            caption = item['query']
            caption = caption[0].lower() + caption[1:]

            if len(item['relevant_windows']) == 1:
                ts = item['relevant_windows'][0]
                answer = f'From <{ts[0]}> to <{ts[1]}>.'
            else:
                answer = []
                for ts in item['relevant_windows']:
                    answer.append(f'<{ts[0]}> to <{ts[1]}>')
                answer = ', '.join(answer)
                answer = 'From ' + answer + '.'
