...
class MixPretrain(Dataset):
    def __init__(
        ...
        self.webvid_video_path = webvid_video_path
        self.panda_video_path = panda_video_path
        self.internvid_video_path = internvid_video_path
        self.num_frames = num_frames
        self.num_segs = num_segs
        self.num_temporal_tokens = num_temporal_tokens
        self.sample = sample

        self.webvid_data = load_json(webvid_anno_path)
        self.panda_data = load_json(panda_anno_path)
        self.internvid_data = load_json(internvid_anno_path)
        ...
        for item in self.webvid_data:
            ...
            self.dataset_names.append('webvid')

        for item in self.panda_data:
            ...
            self.text_inputs.append(self.chat_template.encode(conversations))
            self.dataset_names.append('panda')

        for item in self.internvid_data:
            ...
            self.text_inputs.append(self.chat_template.encode(conversations))
            self.dataset_names.append('internvid')
    ...
    def __getitem__(self, index):
        ...
        if dataset_name == 'webvid':
            video_path = os.path.join(self.webvid_video_path, video_file)
        elif dataset_name == 'panda':
            video_path = os.path.join(self.panda_video_path, video_file)
        elif dataset_name == 'internvid':
            video_path = os.path.join(self.internvid_video_path, video_file)
...
# dataset = MixPretrain(llm='llama3')
# for i in range(10):
